...
def render_block(
    ...
    """Renders a single manifest block into a workflow- and style-specific image directory."""
    cid = block["chunk_id"]
    ...
    target_slug = f"{workflow_slug}__{style_slug}" if style_slug else workflow_slug
    target_name = f"{workflow_name} ({style_name})" if style_name else workflow_name

    output_dir = os.path.join(project_dir, "images", target_slug)
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"{cid}.png")
    rel_img_path = f"images/{target_slug}/{cid}.png"

    print(f"\n[*] Rendering {cid} [{target_name}] ({width}x{height}, seed={seed}) ...")
    ...
    if style_name:
        illus["style_name"] = style_name
    if style_slug:
        illus["style_slug"] = style_slug

    # Preserve in multi-workflow/style map
    if "illustrations" not in block or not isinstance(block["illustrations"], dict):
        block["illustrations"] = {}
    block["illustrations"][target_name] = dict(illus)
    if target_slug != target_name:
        block["illustrations"][target_slug] = dict(illus)
    if not style_slug:
...
def run_phase_2(
    ...
    # Resolve style identity
    if not style_name and not style_slug:
        act_style = manifest.get("active_style")
        if isinstance(act_style, dict):
            style_name = act_style.get("name")
            style_slug = act_style.get("id")

    target_key = f"{workflow_name} ({style_name})" if style_name else workflow_name
    target_slug = f"{workflow_slug}__{style_slug}" if style_slug else workflow_slug
    ...
    for b in blocks:
        if b.get("illustration") is not None:
            ...
            if target_key not in b["illustrations"] and target_slug not in b["illustrations"]:
                new_illus = dict(b["illustration"])
                new_illus["status"] = "pending"
                new_illus["seed"] = random.randint(1, 1125899906842624)
                new_illus["image_file"] = f"images/{target_slug}/{b['chunk_id']}.png"
                new_illus["workflow"] = workflow_name
                if style_name: new_illus["style_name"] = style_name
                if style_slug: new_illus["style_slug"] = style_slug
                b["illustrations"][target_key] = new_illus
                if target_slug != target_key:
                    b["illustrations"][target_slug] = new_illus
            elif target_key not in b["illustrations"] and target_slug in b["illustrations"]:
                b["illustrations"][target_key] = b["illustrations"][target_slug]
            elif target_slug not in b["illustrations"] and target_key in b["illustrations"]:
                b["illustrations"][target_slug] = b["illustrations"][target_key]
            b["illustration"] = b["illustrations"][target_key]
    ...
    if force_all:
        print(f"[*] Force re-render requested for all illustrations with '{target_key}'.")
        for b in blocks:
            if b.get("illustration"):
                b["illustrations"][target_key]["status"] = "pending"
                b["illustrations"][target_key]["seed"] = random.randint(1, 1125899906842624)
                b["illustration"] = b["illustrations"][target_key]
    elif target_cids:
        print(f"[*] Targeted rerun requested for chunks: {sorted(target_cids)} with '{target_key}'.")
        for b in blocks:
            cid = b.get("chunk_id")
            if cid in target_cids and b.get("illustration"):
                ...
                b["illustrations"][target_key]["seed"] = random.randint(1, 1125899906842624)
                b["illustration"] = b["illustrations"][target_key]
                # Only delete the file for this workflow/style set, preserving other outputs
                wf_img = os.path.join(project_dir, "images", target_slug, f"{cid}.png")
                if os.path.isfile(wf_img):
                    try:
                        os.remove(wf_img)
                        print(f"[*] Removed existing image for {cid} [{target_key}]")
                    except Exception as e:
                        print(f"[!] Warning: could not delete {wf_img}: {e}")

    # Identify pending blocks for this specific workflow/style set
    pending_blocks = [
        b for b in blocks
        if b.get("illustrations", {}).get(target_key, {}).get("status") == "pending"
    ]

    total_illus = sum(1 for b in blocks if b.get("illustration") is not None)
    completed_before = total_illus - len(pending_blocks)

    print(f"[*] Manifest status for '{target_key}': {total_illus} total illustration(s). {completed_before} completed, {len(pending_blocks)} pending.")

    if not pending_blocks:
        print(f"[+] All illustrations for '{target_key}' are already completed! Nothing to render.")
        save_manifest_atomic(manifest_path, manifest)
        return manifest

    for idx, block in enumerate(pending_blocks, 1):
        cid = block["chunk_id"]
        print(f"\n--- Processing {idx}/{len(pending_blocks)}: {cid} [{target_key}] ---")
        try:
            # Ensure block['illustration'] is pointing to this workflow/style
            block["illustration"] = block["illustrations"][target_key]
            render_block(
                image_client,
                project_dir,
                block,
